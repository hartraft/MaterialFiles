...
class AddStorageDialogFragment : AppCompatDialogFragment() {
        ...
        MaterialAlertDialogBuilder(requireContext(), theme)
            ...
            .apply {
                ...
                setItems(items) { _, which ->
                    val intent = STORAGE_TYPES[which].second
                    if (intent != null) {
                        startActivitySafe(intent)
                        finish()
                    } else {
                        AddS3ServerDialogFragment().show(parentFragmentManager, "add_s3_server")
                    }
                }
            }
            .create()
    ...
    companion object {
        private val STORAGE_TYPES = listOfNotNull(
            ...
            R.string.storage_add_storage_document_tree to AddDocumentTreeActivity::class.createIntent(),
            ...
            R.string.storage_add_storage_webdav_server to
                EditWebDavServerActivity::class.createIntent().putArgs(EditWebDavServerFragment.Args()),
            R.string.storage_add_storage_s3_server to null,
        )
    }
}
